...
- **通信品質解析**: パケットロス率 (%)、RFC 3550 準拠ジッター (ms)、最大瞬断時間 (ms)、閾値別瞬断回数のリアルタイム算出。
...
### `system/config.json`
システム全体の監視・通知・保守パラメータ。UI設定からの変更が即座に同期・保存されます。
...
| **Webhook が届かない** | URL 誤りまたはネットワーク制限 | 設定の「🔔 テスト送信」でエラーメッセージを確認 |
...
1. **最大50台規模のスケーラビリティ対応**: 非同期並列 Ping（`SendPingAsync`）および 10秒バッファリング一括書込により、最大50台規模の監視時でも低CPU負荷・低ディスクIOで安定稼働します。
2. **超高頻度モード（0.1s）の対象台数**: PC負荷および通信帯域の最適化のため、0.1s 監視対象は最大 2 台までとなります（他機器は 5.0s 間隔でバックグラウンド継続）。
